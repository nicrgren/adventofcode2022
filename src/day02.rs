...
impl Strategy {
    fn score(&self) -> u64 {
        self.you.outcome(self.opponent).score() + self.you.score()
    }
}
...
impl str::FromStr for Shape {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        match s.trim() {
            ...
            s => Err(format!("Invalid shape `{}`", s.trim())),
        }
    }
}
...
impl str::FromStr for Outcome {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        match s.trim() {
            ...
            s => Err(format!("Invalid Outcome `{}`", s.trim())),
        }
    }
}
...
pub fn solve_part1(s: &str) -> u64 {
    parse_input(s).map(|strat| strat.score()).sum()
}

fn parse_input(s: &str) -> impl Iterator<Item = Strategy> + '_ {
    s.lines().map(|line| {
        line.trim()
            .parse::<Strategy>()
...
#[cfg(test)]
mod tests {
    use super::{Shape, Strategy};

    #[test]
    fn parse_strategy() {
        let strat = "A X".parse::<Strategy>().expect("parsing strategy");
        assert_eq!(Shape::Rock, strat.opponent);
        assert_eq!(Shape::Rock, strat.you);
    }
}
